...
- **Environment & Topology**: Windows development PC (`DESKTOP-PE3TPJN`). The monorepo contains the central signaling server, reference web operator peer, and experimental KMP operator peer. Android terminal test client lives in a separate repository (`android-anti-spoofing-lab`).
...
```mermaid
flowchart LR
    OW["Operator Web Peer\n(:3019 Vinext/React 19)"]
    KO["KMP Operator Peer\n(Compose Desktop)"]
    SS["Signaling Server\n(:8080 Node.js /ws)"]
    ...
    OW <-->|"P2P WebRTC Audio/Video\n(Direct LAN)"| AD
    KO -.->|"P2P WebRTC (Future)"| AD
```
...
| `operator-web/` | Reference browser operator peer (React 19, Vinext/Vite, Tailwind) | `operator-web/app/page.tsx` | [operator-web/AGENTS.md](file:///C:/Users/Unionbiometrics/Desktop/company/11.server/ubio-webrtc/operator-web/AGENTS.md) | `components/operator-web.md` |
| `kmp-operator/` | Experimental Kotlin Multiplatform operator (Compose Desktop/Android) | `kmp-operator/desktopApp/src/main/kotlin/com/sumas/operator/main.kt` | [kmp-operator/AGENTS.md](file:///C:/Users/Unionbiometrics/Desktop/company/11.server/ubio-webrtc/kmp-operator/AGENTS.md) | `technical/kmp-operator-migration-plan.md` |
...
| --- | --- | --- | --- |
| Update signaling protocol / event format | `technical/architecture.md` | `signaling-server/src/server.js` | `operator-web/app/page.tsx` → `kmp-operator/shared/.../SignalingMessage.kt` |
| Modify operator web UI / WebRTC flow | `components/operator-web.md` | `operator-web/app/page.tsx` | `operator-web/tests/rendered-html.test.mjs` |
| Advance KMP operator migration step | `technical/kmp-operator-migration-plan.md` | `kmp-operator/shared/.../OperatorReducer.kt` | `kmp-operator/desktopApp/.../DesktopOperatorManager.kt` |
...
| Investigate unverified audio/video/network issues | `issues/needs-verification.md` | `operator-web/app/page.tsx` | Physical device test log in wiki |
